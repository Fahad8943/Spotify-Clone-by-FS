...
// Listen to Events
// Listen to audio runtime events to update progress tracking and text counters
audioElement.addEventListener('timeupdate', () => {
    // 1. Update the sleek visual tracking seekbar progress percentage
    if (!isNaN(audioElement.duration)) {
        let progress = parseInt((audioElement.currentTime / audioElement.duration) * 100);
        ProgressBar.value = progress;
    }

    // 2. Calculate and format Current Time Displayed minute and seconds
    ...
    // 3. Calculate and format Total Track Duration Displayed
    if (audioElement.duration) {
        ...
        document.getElementById('totalDuration').innerText = `${totalMins}:${totalSecs}`;
    } else {
